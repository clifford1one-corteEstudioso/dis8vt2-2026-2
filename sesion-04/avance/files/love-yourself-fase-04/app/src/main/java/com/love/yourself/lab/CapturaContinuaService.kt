...
class CapturaContinuaService : Service() {
    ...
    private var overlay: VentanaOverlay? = null

    /**
     * Instantes de los eventos sobre umbral dentro de la ventana movil. Es una
     * lista corta: a 4 muestras por segundo y 15 segundos de ventana, nunca pasa
     * de unas decenas de elementos.
     */
    private val eventosRecientes = ArrayDeque<Long>()
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        overlay = VentanaOverlay(this).also { ventana ->
            // Si falta el permiso de dibujar encima, la medicion igual sirve:
            // el CSV es el dato duro, el overlay es el sello visual.
            try {
                ventana.mostrar()
            } catch (e: Exception) {
                Log.e(TAG, "No se pudo mostrar el overlay; falta el permiso?", e)
            }
        }
    ...
    /**
     * Ritmo de los ultimos VENTANA_S segundos. El acumulado de una sesion larga
     * se aplana y deja de decir nada; lo que importa es la velocidad actual.
     */
    private fun refrescarOverlay(ahora: Long) {
        val corte = ahora - VentanaOverlay.VENTANA_S * 1000L
        while (eventosRecientes.isNotEmpty() && eventosRecientes.first() < corte) {
            eventosRecientes.removeFirst()
        }
        val transcurrido = (ahora - inicioMs) / 1000
        // La ventana no esta llena al principio: dividir por 15 daria un ritmo
        // falsamente bajo en los primeros segundos.
        val ventana = minOf(VentanaOverlay.VENTANA_S.toLong(), maxOf(1L, transcurrido))
        val ritmo = eventosRecientes.size.toDouble() / ventana
        mainHandler.post { overlay?.actualizar(transcurrido, ritmo) }
    }
